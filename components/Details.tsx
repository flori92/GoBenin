    ...
    <div className={`min-h-screen pb-24 relative z-50 transition-colors duration-300 ${theme === 'dark' ? 'bg-background-dark' : 'bg-gray-50'}`}>
      ...
      <div className="relative h-[420px] w-full shrink-0">
        <div className="h-full w-full bg-gray-200 bg-cover bg-center" style={{ backgroundImage: `url('${location.image}')` }}>
          <div className={`absolute inset-0 bg-gradient-to-t via-transparent to-transparent ${theme === 'dark' ? 'from-background-dark' : 'from-gray-50'}`}></div>
        </div>
      ...
      {/* Content Body */}
      <div className={`relative -mt-4 flex flex-1 flex-col rounded-t-3xl px-5 pt-6 ${theme === 'dark' ? 'bg-background-dark' : 'bg-gray-50'}`}>
        ...
        {location.hours && (
          <div className="grid grid-cols-3 gap-3 mb-8">
            <div className={`flex flex-col items-center justify-center rounded-2xl p-3 shadow-sm border ${theme === 'dark' ? 'bg-[#2c241b] border-gray-800' : 'bg-white border-gray-100'}`}>
              ...
              <span className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{location.hours}</span>
            </div>
            <div className={`flex flex-col items-center justify-center rounded-2xl p-3 shadow-sm border ${theme === 'dark' ? 'bg-[#2c241b] border-gray-800' : 'bg-white border-gray-100'}`}>
              ...
              <span className={`text-sm font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{location.duration}</span>
            </div>
            <div className={`flex flex-col items-center justify-center rounded-2xl p-3 shadow-sm border ${theme === 'dark' ? 'bg-[#2c241b] border-gray-800' : 'bg-white border-gray-100'}`}>
          ...
          <div className="mb-8">
             ...
             <div className="grid grid-cols-4 gap-4">
               {location.features.map((f, i) => (
                  <div key={i} className={`flex flex-col items-center gap-2 rounded-xl p-3 shadow-sm border ${theme === 'dark' ? 'bg-[#2c241b] border-white/5' : 'bg-white border-black/5'}`}>
                    <span className="material-symbols-outlined text-primary">{f.icon}</span>
                    <span className={`text-[10px] font-medium ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{f.label}</span>
                  </div>
               ))}
             </div>
          </div>
        ...
        {/* About Section */}
        <div className="mb-8">
          <h2 className={`mb-3 text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{t('about_dest')}</h2>
          <div className="relative">
            <p className={`text-sm leading-relaxed ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>{location.description}</p>
        ...
        <div className="mb-8">
          <div className="flex items-center justify-between mb-4">
            <h2 className={`text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{t('reviews')}</h2>
            <button className="text-sm font-semibold text-primary">{t('see_all')}</button>
          </div>
          <div className={`rounded-2xl p-5 shadow-sm border ${theme === 'dark' ? 'bg-[#2c241b] border-gray-800' : 'bg-white border-gray-100'}`}>
            <div className="flex items-center gap-4">
              <div className="flex flex-col items-center">
                <span className={`text-4xl font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{location.rating}</span>
              ...
              <div className={`h-12 w-px ${theme === 'dark' ? 'bg-gray-700' : 'bg-gray-200'}`}></div>
        ...
        {/* What's Nearby Map */}
        <div className="mb-6">
          <h2 className={`mb-4 text-lg font-bold ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>{t('location')}</h2>
          <div className={`relative overflow-hidden rounded-2xl h-64 group cursor-pointer ${theme === 'dark' ? 'bg-gray-800 border-gray-700' : 'bg-gray-100 border-gray-200'} border`}>
            ...
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 flex flex-col items-center z-10">
             ...
             <div className="absolute bottom-4 left-4 right-4 z-10">
                <button className={`w-full rounded-xl backdrop-blur-md py-3 text-sm font-bold shadow-lg ${theme === 'dark' ? 'bg-[#2c241b]/90 text-white' : 'bg-white/90 text-slate-900'}`}>
                    {t('explore_map')}
                </button>
            </div>
          </div>
      ...
      {/* Sticky Bottom CTA */}
      <div className={`fixed bottom-0 left-0 right-0 z-50 w-full border-t px-6 py-4 backdrop-blur-lg ${theme === 'dark' ? 'border-white/10 bg-charcoal-dark/95' : 'border-gray-200 bg-white/95'}`}>
